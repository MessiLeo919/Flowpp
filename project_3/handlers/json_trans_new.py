import json
def json_transfer_action_service(file):
    with open(file,"r") as f:
        load_dict = json.load(f)         #转化为字典
        print("加载入文件完成...")
        print('----------------')
        for list_class in load_dict.keys():
            if list_class =="action" or "service":
                app_L_1 = []
                dict_All_1 = []
    ...
    return [app_L_1,dict_All_1]
def json_transfer_application(file):
    with open(file,"r") as f:
        load_dict = json.load(f)         #转化为字典
        print("加载入文件完成...")
        print('----------------')
        for list_class in load_dict.keys():
            if list_class =="application":
                app_L_2 = []
                dict_All_2 = []
                for i in load_dict[list_class]: #i为大类下面每一个字典
                    b = i['domain']
